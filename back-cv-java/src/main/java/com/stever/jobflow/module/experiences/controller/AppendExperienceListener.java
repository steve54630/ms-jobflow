    ...
    public void subscribe() {
        ...
        d.subscribe("cv.experiences.add", m -> {
            try {
                ...
                log.info("Ajout de l'expérience {} au cv {}", data.getExperience().get(0).getId(), data.getId());
                cvService.verifyOwnership(data.getId(), data.getSub());
                CvSchema updated = experienceService.appendToField(data.getId(), CVFields.EXPERIENCES, data.getExperience().get(0));
