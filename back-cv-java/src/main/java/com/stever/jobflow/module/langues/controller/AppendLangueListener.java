    ...
    public void subscribe() {
        ...
        d.subscribe("cv.langues.add", m -> {
            try {
                ...
                log.info("Ajout de la langue {} au cv {}", data.getLangue().get(0).getId() , data.getId());
                cvService.verifyOwnership(data.getId(), data.getSub());
                CvSchema updated = langueService.appendToField(data.getId(), CVFields.LANGUES, data.getLangue().get(0));
