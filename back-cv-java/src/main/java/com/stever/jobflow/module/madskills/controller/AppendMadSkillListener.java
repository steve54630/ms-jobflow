    ...
    public void subscribe() {
        ...
        d.subscribe("cv.mad_skills.add", m -> {
            try {
                ...
                log.info("Ajout du mad skill {} au cv {}", data.getMadSkill().get(0).getId(), data.getId());
                cvService.verifyOwnership(data.getId(), data.getSub());
                CvSchema updated = madSkillService.appendToField(data.getId(), CVFields.MADSKILLS, data.getMadSkill().get(0));
