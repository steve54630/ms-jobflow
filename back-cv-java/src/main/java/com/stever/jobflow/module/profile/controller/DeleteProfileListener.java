...
@Slf4j
@Component
public class DeleteProfileListener extends BaseListener {

    private final ProfileService profileService;
    private final CvService cvService;
    private final Connection ns;

    public DeleteProfileListener(ObjectMapper mapper, ErrorPublisher errorPublisher, ProfileService profileService, CvService cvService, Connection ns) {
        super(mapper, errorPublisher);
        this.profileService = profileService;
        this.cvService = cvService;
        this.ns = ns;
        log.info("DeleteProfileListener initialized");
    }

    @PostConstruct
    public void subscribe() {
        Dispatcher d = ns.createDispatcher();
        d.subscribe("cv.profile.delete", m -> {
            try {
                EnvelopeRequest<ProfileDto> request = parseRequest(m, new TypeReference<>() {
                });
                ProfileDto data = request.getData();
                log.info("Suppression du champ {} pour le cv {} ", data.getSelect(), data.getValue());
                cvService.verifyOwnership(data.getId(), data.getSub());
                CvSchema updated = profileService.removeFromField(data.getId(), data.getSelect(), data.getValue());
