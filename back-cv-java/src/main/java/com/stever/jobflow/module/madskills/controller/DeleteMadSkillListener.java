    ...
    public void subscribe() {
        ...
        d.subscribe("cv.langues.delete", m -> {
            try {
                ...
                log.info("Suppression de la langue {} du cv {}", data.getId(), data.getLanguageId());
                cvService.verify(data.getId(), data.getSub());
                CvSchema updated = langueService.removeFromField(data.getId(), CVFields.LANGUAGES, data.getLanguageId());
